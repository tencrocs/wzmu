...
void CraftSystem::ReadStageData(char * File)
{
	...
	if( !SMDFile )
	{
		MessageBox(NULL, L"file not found", L"[CraftSystem]", ERROR);
		return;
	}
	// ----
	while(true)
...
{
	if( gObjUser.lpPlayer->MoneyInventory < this->m_StageData[gObjUser.m_CraftStage].QuestData[gObjUser.m_CraftQuest].ReqMoney )
	{
		return false;
	}
	// ----
